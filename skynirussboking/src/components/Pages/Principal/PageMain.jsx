...
function Main(){
  ...
  const { currentStatus, currentHour, currentGate, currentDelay, currentAirline } = useContext(PageContext)
      ...
      <section className="flex flex-col gap-25">
        ...
        <div className="grid grid-cols-4 w-auto mx-auto justify-items-center content-end">
          ...
          <section className="grid border rounded-2xl w-fit h-145 text-white items-center">
            <StatusFlight isActive={currentStatus}/>
            <HourEstimate currentHour={currentHour}/>
            <Gate currentGate={currentGate} />
            <Delay currentDelay={currentDelay} />
            <Airlines currentAirline={currentAirline} />
          </section>
        </div>
      </section>
    </main>
